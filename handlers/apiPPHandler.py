...
class handler(requestsManager.asyncRequestHandler):
	...
	def asyncGet(self, uri):
		...
		try:
			...
			osuapiData = osuapiHelper.osuApiRequest("get_beatmaps", f"b={beatmapID}")
			if not osuapiData or "file_md5" not in osuapiData or "beatmapset_id" not in osuapiData:
			...
			if gameMode in (gameModes.STD, gameModes.TAIKO):
				...
				if accuracy < 0 and modsEnum == 0:
					...
					log.debug(f"Specific request ({accuracy}%/{modsEnum}). Calculating pp with oppai...")
					oppai = rippoppai.oppai(bmap, mods=modsEnum, tillerino=True)
					bmap.starsStd = oppai.stars
					if accuracy > 0:
