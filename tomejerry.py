...
class SimpleRecalculator(Recalculator):
    ...
    def offset_ids_query(self, limit: int, offset: int) -> str:
        return self.ids_query.query + f" LIMIT {limit} OFFSET {offset}"
...
class ScoreIdsPool:
    ...
    def load(self, recalculator: Recalculator):
        ...
        self.logger.debug(f"Loaded {len(self.scores)} scores")
    ...
    def chunk(self, chunk_size: int) -> List[int]:
        ...
        self.logger.debug(f"Chunked {chunk_size} scores. Current scores in pool: {len(self.scores)}")
...
class Worker:
    ...
    def __init__(self, chunk_size: int, worker_id: int=-1, start: bool=True):
        ...
        self.logger: logging.Logger = logging.getLogger(f"w{worker_id}")
    ...
    def recycle(self, start: bool=True):
        ...
        self.logger.debug(f"Recycled with {self.chunk_size} new scores")
        if start:
            self.threaded_work()
    ...
    def _work(self):
        ...
        self.logger.info(f"Started worker. Assigned {self.chunk_size} scores")
        try:
        ...
        try:
            ...
            for i, lw_score in enumerate(self.scores):
                if i % self.log_every == 0:
                    self.logger.debug(f"Processed {i}/{self.chunk_size} scores")
    ...
    def save_recalculations(self):
        ...
        for i, lw_score in enumerate(self.scores):
            if i % self.log_every == 0:
                self.logger.debug(f"Updated {i}/{self.chunk_size} scores")
    ...
    def log_failed_score(self, score_: Dict[str, Any], additional_information: str="", traceback_: bool=False):
        ...
        msg = ""
        if traceback_:
            msg = f"\n\n\nUnhandled exception: {sys.exc_info()}\n{traceback.format_exc()}"
        msg += f"score_id:{score_['id']} ({additional_information})".strip()
...
def mass_recalc(recalculator: Recalculator, workers_number: int=MAX_WORKERS, chunk_size: Optional[int]=None):
    ...
    logging.info(f"Query: {recalculator.ids_query.query} ({recalculator.ids_query.parameters})")
    ...
    FAILED_SCORES_LOGGER.addHandler(
        logging.FileHandler(f"tomejerry_failed_scores_{time.strftime('%d-%m-%Y--%H-%M-%S')}.log")
    )

    # Get the number of total scores from the result dict
    total_scores = total_scores[next(iter(total_scores))]
    logging.info(f"Total scores: {total_scores}")
    if total_scores == 0:
        return
    ...
    logging.info(f"Using {workers_number} workers and {scores_per_worker} scores per worker")
    ...
    with progressbar.ProgressBar(
        ...
        while True:
            ...
            widgets[3] = f" w_pp:<{len([x for x in workers if x.status == WorkerStatus.RECALCULATING])}/{len(workers)}>"
            widgets[4] = f" w_db:<{len([x for x in workers if x.status == WorkerStatus.SAVING])}/{len(workers)}>"
            widgets[5] = f" w_done:<{len(workers_done)}/{len(workers)}>"
            widgets[6] = f" rec:{recycles}"
    ...
    logging.info(
         "\n\nDone!\n"
        f":: Recalculated\t{total_scores - failed_scores} scores\n"
        f":: Failed\t{failed_scores} scores\n"
        f":: Total\t{total_scores} scores\n\n"
...
def main():
   ...
   #recalc_group.add_argument("-c",   "--cmyui",     help="calculates pp with cmyui flag",      required=False, action="store_true")
    recalc_group.add_argument("-r",   "--recalc",    help="calculates pp for all high scores",  required=False, action="store_true")
    recalc_group.add_argument("-z",   "--zero",      help="calculates pp for 0 pp high scores", required=False, action="store_true")
    recalc_group.add_argument("-i",   "--id",        help="calculates pp for the score with this score_id",        required=False)
    recalc_group.add_argument("-m",   "--mods",      help="calculates pp for high scores with these mods (flags)", required=False)
    recalc_group.add_argument("-g",   "--gamemode",  help="calculates pp for scores played on this game mode (std:0, taiko:1, ctb:2, mania:3)", required=False)
    recalc_group.add_argument("-u",   "--userid",    help="calculates pp for high scores set by a specific user (user_id)", required=False)
    recalc_group.add_argument("-b",   "--beatmapid", help="calculates pp for high scores played on a specific beatmap (beatmap_id)", required=False)
    recalc_group.add_argument("-fhd", "--fixstdhd",  help="calculates pp for std hd high scores (14/05/2018 pp algorithm changes)",  required=False, action="store_true")

    parser.add_argument("-w",  "--workers",   help=f"number of workers. {MAX_WORKERS // 2} by default. Max {MAX_WORKERS}", required=False)
    ...
    logging.info(f'Running under {"UNIX" if UNIX else "WIN32"}')
    ...
    recalculators_gen = {
       ...
       #"cmyui":     lambda: SimpleRecalculator(("scores.completed = 3", "pp > 69", "pp < 69")),
