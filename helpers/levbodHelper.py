...
def levbodRequest(handler, params=None):
	if params is None:
		params = {}
	result = get("{}/{}".format(glob.conf.config["levbod"]["url"], handler), params=params)

	try:
		data = loads(result.text)
	except (JSONDecodeError, ValueError, RequestException, KeyError, exceptions.noAPIDataError):
		return None
