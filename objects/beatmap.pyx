...
class beatmap:
	...
	def addBeatmapToDB(self):
		...
		if bdata is not None:
			...
			if frozen: self.rankedStatus = bdata["ranked"]
			log.debug(f'Deleting old beatmap data ({bdata["id"]})')
			glob.db.execute("DELETE FROM beatmaps WHERE id = %s LIMIT 1", [bdata["id"]])
		else: frozen = 0 # Unfreeze beatmap status

		# Add new beatmap data
		log.debug("Saving beatmap data in db...")

		try:
			if int(self.bpm) > 2147483647:
				raise ValueError
		except ValueError:
			log.info(f"SAVED beatmap {self.songName} {self.beatmapID}] from devestation", discord='cm')
			self.bpm = 0
	...
	def setDataFromDB(self, md5):
		...
		if int(expire) > 0 and time() > data["latest_update"] + int(expire):
	...
	def setDataFromOsuApi(self, md5, beatmapSetID):
		...
		mainData = None
		dataStd = osuapiHelper.osuApiRequest("get_beatmaps", f"h={md5}&a=1&m=0")
		dataTaiko = osuapiHelper.osuApiRequest("get_beatmaps", f"h={md5}&a=1&m=1")
		dataCtb = osuapiHelper.osuApiRequest("get_beatmaps", f"h={md5}&a=1&m=2")
		dataMania = osuapiHelper.osuApiRequest("get_beatmaps", f"h={md5}&a=1&m=3")
		...
		if mainData is None:
			...
			dataStd = osuapiHelper.osuApiRequest("get_beatmaps", f"s={beatmapSetID}&a=1&m=0")
			dataTaiko = osuapiHelper.osuApiRequest("get_beatmaps", f"s={beatmapSetID}&a=1&m=1")
			dataCtb = osuapiHelper.osuApiRequest("get_beatmaps", f"s={beatmapSetID}&a=1&m=2")
			dataMania = osuapiHelper.osuApiRequest("get_beatmaps", f"s={beatmapSetID}&a=1&m=3")
			if dataStd is not None:
				mainData = dataStd
			elif dataTaiko is not None:
				mainData = dataTaiko
		...
		self.songName = f'{mainData["artist"]} - {mainData["title"]} [{mainData["version"]}]'
		self.fileMD5 = md5
	...
	def getData(self, totalScores=0, version=4):
		...
		if self.rankedStatus == rankedStatuses.PENDING and generalUtils.stringToBool(glob.conf.config["extra"]["unrank_leaderboard"]) == True:
		...
		if self.rankedStatus != rankedStatuses.NOT_SUBMITTED and self.rankedStatus != rankedStatuses.NEED_UPDATE and self.rankedStatus != rankedStatuses.UNKNOWN:
			...
			data += f"|{self.beatmapID}|{self.beatmapSetID}|{totalScores}\n{self.offset}\n{self.songName}\n{self.rating}\n"
	...
	def getCachedTillerinoPP(self):
		...
		return [data["pp_100"], data["pp_99"], data["pp_98"], data["pp_95"]] if data else [0, 0, 0, 0]
	...
	def is_rankable(self):
		...
		or (self.rankedStatus == rankedStatuses.PENDING and generalUtils.stringToBool(glob.conf.config["extra"]["unrank_leaderboard"]) == True)
